...
if( !empty($_POST["mainID"]) && !empty($_POST["btnTabMonth"]) ) {
    ...
    if( (isset($_SESSION['tk']['u_id'])) && ($_SESSION['tk']['u_id'] == '1' || $_SESSION['tk']['u_id'] == '8' || $_SESSION['tk']['u_id'] == '1579926' || $_SESSION['tk']['u_id'] == '1581081' || $_SESSION['tk']['u_id'] == '3') ){
        ...
        $sqlInsert = " INSERT INTO tk_sales_deposit (main_id, month, date, item, amount, note, row_no) VALUES ('".trim($_POST["mainID"])."', '".trim($_POST["btnTabMonth"])."', '".trim($_POST["today"])."', 'kosong', 'kosong', 'kosong', '".$ThisRow."') ";
        if ( ($conn->query($sqlInsert) === TRUE) ) {
            ...
            $header = date('d/m/Y H:i:s')." | ADD ( Deposit - ".trim($_POST["btnTabMonth"])." ".$ThisRow.") | ".$adminName."\n";
            if( $header != '' ){
                /*$myFile = $fileName;
                $message = $header;
