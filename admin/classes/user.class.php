...
class user extends db {
    ...
    public function GetUserJobAddLinkNew($user_role = NULL, $search_email = NULL){

            $qry = "SELECT * FROM tk_user AS U 
                INNER JOIN ".DB_PREFIX."_user_details AS UD ON U.u_id = UD.ud_u_id
                WHERE 1
                AND U.u_id = '".$search_email."'
                AND U.u_role = '".$user_role."'";
    ...
    public function isTutor($data){
        ...
        $search_phone_number    = isset($data['ud_phone_number'])           ?  $this->RealEscape($data['ud_phone_number']) : '';//phonenumber
         // if ($search_tutor == 'Yes') {
            ...
            $validatedaa    = isset($data['validatedaa'])           ? $data['validatedaa'] : ''; 
...
if ( $level_taught != '' || $subject_check != '' ) {
    ...
    INNER JOIN ".DB_PREFIX."_tution_course   AS TC  ON TC.tc_id     = TRS.trs_tc_id
    INNER JOIN ".DB_PREFIX."_tution_subject   AS TS  ON TS.ts_id     = TRS.trs_ts_id";*/
    $sql .= " INNER JOIN ".DB_PREFIX."_tutor_subject AS TRS ON TRS.trs_u_id = U.u_id  ";
}
...
if ( $city_check != '' ) {
	...
	$sql .= " INNER JOIN ".DB_PREFIX."_tutor_area_cover AS TAC ON TAC.tac_u_id = U.u_id ";
    ...
    // GENDER
        ...
        if ($searchConductOnline != '') {
            if ($searchConductOnline == 'yes') {
                ...
                if ($hiddentoolsname != '') {
                    $searchForValue = ',';
                    if( strpos($hiddentoolsname, $searchForValue) !== false ) {
                         //echo "Found";
                         $sql .= " AND conduct_online_text = '".$hiddentoolsname."' ";
                    }else{
                        $sql .= " AND conduct_online_text LIKE '%".$hiddentoolsname."%' ";
                    }
                }else{
                     $sql .= " AND conduct_online_text != '' ";
                }
...
if ($validatedaa != '') {
    if ($validatedaa == 'paid') {
        $sql .= " AND U.u_role = '3' AND U.u_email IN (SELECT j_hired_tutor_email FROM ".DB_PREFIX."_job WHERE j_payment_status = 'paid') ";
    }
    if ($validatedaa == 'unpaid') {
        $sql .= " AND U.u_role = '3' AND U.u_email IN ( (SELECT j_hired_tutor_email FROM ".DB_PREFIX."_job WHERE j_payment_status = 'pending' AND j_hired_tutor_email  NOT IN  (SELECT j_hired_tutor_email FROM ".DB_PREFIX."_job WHERE j_payment_status = 'paid'))  )";
    }
    if ($validatedaa == 'none') {
        $sql .= "  AND U.u_role = '3' AND U.u_email NOT IN (SELECT j_hired_tutor_email FROM ".DB_PREFIX."_job) ";
    }
...
if ( $level_taught != '' || $subject_check != '' || $city_check != '' ) {
    $sql .= "GROUP BY U.u_username ";  
        ...
        if ( $subject_check != '' ) {
			$countSubjek = count($subject_check);     
        }
		
        if ( $city_check != NULL && $subject_check == NULL ) {
            $sql .= "HAVING COUNT(distinct TAC.tac_city_id) = '".$countcity."'";
        }else if ( $subject_check != NULL && $city_check == NULL ) {
            $sql .= "HAVING COUNT(distinct TRS.trs_ts_id) = '".$countSubjek."'";
        }else if ( $city_check != NULL && $subject_check != NULL ) {
			$sql .= "HAVING COUNT(distinct TAC.tac_city_id) = '".$countcity."' AND COUNT(distinct TRS.trs_ts_id) = '".$countSubjek."' ";
        }
...
// luqman
    ...
    public function LoadProfileUserBM($data){
        $userid             = isset($data['userid'])                  ? $data['userid'] : '';
        $displayid             = isset($data['displayid'])                  ? $data['displayid'] : '';  

        if($userid != ''){
            $sql = "SELECT TRS.trs_u_id, GROUP_CONCAT(TS.ts_description SEPARATOR ', ') ts_description, TC.tc_description FROM tk_tutor_subject TRS
                    INNER JOIN tk_tution_subject TS ON TS.ts_id = TRS.trs_ts_id
                    INNER JOIN tk_tution_course TC ON TRS.trs_tc_id = TC.tc_id

                    WHERE TRS.trs_u_id = '".$userid."'
                    GROUP BY TRS.trs_u_id, TC.tc_description
					ORDER BY TRS.trs_tc_id asc";
        }elseif($displayid !=''){
            $sql = "SELECT TRS.trs_u_id, GROUP_CONCAT(TS.ts_description SEPARATOR ', ') ts_description, TC.tc_description FROM tk_tutor_subject TRS
                    INNER JOIN tk_tution_subject TS ON TS.ts_id = TRS.trs_ts_id
        ...
        if ($result->num_rows > 0) {
            while($row = mysqli_fetch_array($result)){
                $tc_title = $row['tc_description'];
                $ts_title = $row['ts_description'];
                $tutor_pro[] = array("tc_title" => $tc_title,"ts_title" => $ts_title);  
            }
            echo json_encode($tutor_pro);
        }else{
	...
	public function LoadArea($data){
        ...
        if($userid != ''){
            $sql = "SELECT tac_u_id, GROUP_CONCAT(city_name SEPARATOR ', ') city_name, st_name, st_name_bm, city_id FROM tk_tutor_area_cover
        ...
        if ($result->num_rows > 0) {
			while($row = mysqli_fetch_array($result)){
				$st_name = $row['st_name'];
				$st_name_bm = $row['st_name_bm'];
				$city_name = $row['city_name'];
         
				$tutorArea[] = array("st_name" => $st_name,"city_name" => $city_name,"st_name_bm" => $st_name_bm);  
			}
...
if($resultRecordCity->num_rows > 0){
    ...
    $outputRecordCity = '';
                    ...
                    $Userinfoarr = array("area" => $area, "state" => $state, "email" => $email, "phoneno" => $phoneno, "admincomment" => date('d/m/Y').' - '.$admincomment, "ud_last_name" => $ud_last_name, "city" => $ud_city);
                }
...
GROUP BY '".$class_user_id."'";
    ...
    function SaveUser($data) {
        ...
        $email              = $data['u_email'];
        $hiddenTutorEmail   = $data['hiddenTutorEmail'];
        ...
        $resit_pv_name       = isset($data['resit_pv_name']) ? $this->RealEscape($data['resit_pv_name']) : '';
   ...
   if (getenv('HTTP_X_FORWARDED_FOR')) {
       ...
       $dataIpAddress = "$ipaddress"; 
        ...
        $Salutation         = $this->RealEscape($data['Salutation']);
		...
		$BankName = isset($data['BankName']) ? $this->RealEscape($data['BankName']) : '';
		$AccNo    = isset($data['AccNo']) ? $this->RealEscape($data['AccNo']) : '';
		...
		$Language   = isset($data['Language']) ? $this->RealEscape($data['Language']) : '';
...
if($resultUserRecord->num_rows > 0){
	...
	if( isset($data['u_status']) ){
		if( $rowUserRecord['u_status'] == $data['u_status'] ){
			...
			}			/*new user status added on 14/10/21 (siti) value is C*/			if($rowUserRecord['u_status'] == 'C'){				$no16 = 'User Status : DO NOT HIRE <br>';			}			else{
...
if($u_role == 3){
	$TutorEmailJob = "SELECT j_id, j_hired_tutor_email FROM ".DB_PREFIX."_job WHERE j_hired_tutor_email = '".$hiddenTutorEmail."' ";
	$reTutorEmailJob = $this->db->query($TutorEmailJob);
	if($reTutorEmailJob->num_rows > 0){
		while($roTutorEmailJob = mysqli_fetch_array($reTutorEmailJob)){
			
			$UpdateHired = " UPDATE ".DB_PREFIX."_job SET j_hired_tutor_email = '".$email."' ";
			$UpdateHired .= " WHERE j_id = '".$roTutorEmailJob['j_id']."' ";
			$exeUpdateHired = $this->db->query($UpdateHired);
...
/* update job email*/
                ...
                /* END - fadhli */
					...
					$sqlGetExp = "SELECT ud_u_id, ud_tutor_experience, ud_tutor_experience_month FROM ".DB_PREFIX."_user_details WHERE ud_u_id = {$id} ";
					$resultGetExp = $this->db->query($sqlGetExp);
					if($resultGetExp->num_rows > 0){
						$rowGetExp = mysqli_fetch_array($resultGetExp);
						if( ($rowGetExp['ud_tutor_experience_month'] != $experienceMonth) || ($rowGetExp['ud_tutor_experience'] != $ud_tutor_experience) ){
							
							if( $_SESSION[DB_PREFIX]['u_id'] == '2' ){
								$thisUser = '[C1]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '3' ){
								$thisUser = '[FM]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '4' ){
								$thisUser = '[C2]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '5' ){
								$thisUser = '[C3]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '6' ){
								$thisUser = '[MM]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '1581040' ){
								$thisUser = '[MM]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '8' ){
								$thisUser = '[CTO]';
							}else if( $_SESSION[DB_PREFIX]['u_id'] == '1579926' ){
								$thisUser = '[AHN]';
							}else{
								$thisUser = '[NOT SET]';
							}
							
							$rateComment = date('d/m/Y').' -'.$thisUser.' Updated Profile (tick 2)';
							$rateComment2 = date('d/m/Y').' -'.$thisUser.' Updated Profile (untick 2)';
							if ( $experienceMonth == 'year' ) {
								if( $ud_tutor_experience >= '3' ){
									$sqlInternalRating = "SELECT ri_tutor, ri_experience, ri_comments FROM ".DB_PREFIX."_review_rating_internal WHERE ri_tutor = {$id} ";
									...
									if($resultInternalRating->num_rows > 0){
										...
										if( $rowInternalRating['ri_experience'] != 'true' ){
    										$allotSql = " UPDATE ".DB_PREFIX."_review_rating_internal SET ri_experience = 'true', ri_comments = '".$newComment."' WHERE ri_tutor = '{$id}' ";
    										if ($this->db->query($allotSql)){} else {}										    
										}

									}else{
										$allotSql = " INSERT INTO ".DB_PREFIX."_review_rating_internal SET ri_tutor = '{$id}', ri_experience = 'true', ri_comments = '{$rateComment}' ";
										if ($this->db->query($allotSql)){} else {}						
									}
								}else{
									$sqlInternalRating = "SELECT ri_tutor, ri_experience, ri_comments FROM ".DB_PREFIX."_review_rating_internal WHERE ri_tutor = {$id} ";
									...
									if($resultInternalRating->num_rows > 0){
										...
										if( $rowInternalRating['ri_experience'] != 'false' ){
    										$allotSql = " UPDATE ".DB_PREFIX."_review_rating_internal SET ri_experience = 'false', ri_comments = '".$newComment."' WHERE ri_tutor = '{$id}' ";
    										if ($this->db->query($allotSql)){} else {}										    
										}

									}else{
										$allotSql = " INSERT INTO ".DB_PREFIX."_review_rating_internal SET ri_tutor = '{$id}', ri_experience = 'false', ri_comments = '{$rateComment2}' ";
										if ($this->db->query($allotSql)){} else {}						
									}
								}
							}else{
...
if (filter_var($lastname, FILTER_VALIDATE_EMAIL)) {
	$this->db->query(" UPDATE ".DB_PREFIX."_job SET actual_email = '".$lastname."' WHERE j_email = '".$email."' ");
                    ...
                    // Session::SetFlushMsg("error", 'Phone number has been used previously.');
						...
						/*START save Parent Banned*/
						if( $u_role == '4' ){
							$sqlUserBanned = "
								UPDATE ".DB_PREFIX."_user SET 
								u_password = '".md5($password)."',
								u_gender = '".$gender."',
								u_status = '".$u_status."',
								u_lang = '".$Language."',
								...
								WHERE u_id = {$id}
								";
							$exeUserBanned = $this->db->query($sqlUserBanned);
							$sqlDetailBanned = "
								...
								WHERE ud_u_id = {$id}
								";
							$exeDetailBanned = $this->db->query($sqlDetailBanned);							
						}
						/*END save Parent Banned*/
						
						/*START save tutor Banned*/
						if( $u_role == '3' ){
							$sqlUserBannedTutor = "
								UPDATE ".DB_PREFIX."_user SET 
								u_displayname = '".$displayname."',  
								...
								WHERE u_id = {$id}
								";
							$exeUserBannedTutor = $this->db->query($sqlUserBannedTutor);
							$sqlDetailBannedTutor = "
								UPDATE ".DB_PREFIX."_user_details SET 
								ud_first_name	= '".$firstname."',
								ud_last_name 	= '".$lastname."',
								ud_dob			= '".$ud_dob."',
								...
								WHERE ud_u_id = {$id}
								";
							$exeDetailBannedTutor = $this->db->query($sqlDetailBannedTutor);	
						}
						/*END save tutor Banned*/
...
/* END - fadhli - DELETE USER LOG*/
    ...
    public function TutorsJobNew($u_id) {
        return $this->db->query("SELECT * FROM ".DB_PREFIX."_job WHERE u_id_tutor = '".$u_id."'");
    }
    public function ClientsJobNew($u_id) {
        return $this->db->query("SELECT * FROM ".DB_PREFIX."_job WHERE u_id = '".$u_id."'");
    }
...
EOD;
    ...
    public function SaveOnlineRate($data) {
        if (isset($data['or_id']) && $data['or_id'] > 0) {

            $sql = "UPDATE ".DB_PREFIX."_online_rates SET 
            
            or_rate = '".$data['or_rate']."' 

            WHERE or_id = '".$data['or_id']."'";

            Session::SetFlushMsg("success", 'Rate Updated Successfully.');
