	...
	if($resUser > 0){
		...
		if(strlen($getTime) == '7'){
		    ...
		    $getTime = str_replace("-",":",$getTime).':00';
                            ...
                            //$queryProof1 = " SELECT * FROM ".DB_PREFIX."_history_terms WHERE ht_pmt_id='80' AND DATE_FORMAT(STR_TO_DATE(ht_date,'%d/%m/%Y'), '%Y-%m-%d') <= '".$dateFormat."' ORDER BY ht_id DESC"; 
                            $queryProof1 = " SELECT * FROM ".DB_PREFIX."_history_terms WHERE ht_pmt_id='80' AND DATE_FORMAT(STR_TO_DATE(ht_date,'%d/%m/%Y'), '%Y-%m-%d') <= '".$dateFormat."' AND ht_time <= '".$getTime."' ORDER BY ht_id DESC"; 
...
try {
  ...
  $mpdf->SetTitle('Proof2 (for group tuition) | tutorkami.com');
  $mpdf->WriteHTML($html);
