...
<?php 
/*
$resAct = $Sys->FetchActivityTypes();  
    while($arrAct = $resAct->fetch_assoc()){
        echo $arrAct['at_name'].'<br/>';
} 
*/
?>
					...
					<tr>
						...
						<td>
						    ...
						    <select name="user" class="form-control">
						        <?PHP
						        $listUsers = $Sys->listUsers();  
						        while($arrAct = $listUsers->fetch_assoc()){
                                    ?><option value="<?php echo $arrAct['username']; ?>"><?php echo $arrAct['username']; ?></option><?PHP
						        } 
						        ?>
						    </select>
	...
	if (isset($_POST['submit'])) { 
		if(isset($_POST['msg']) and isset($_POST['time']) and isset($_POST['loops']) and isset($_POST['loop_every']) and isset($_POST['user'])) {
			...
			$user = $_POST['user']; 
			$isSaved = $Sys->saveNotification($title, $msg,$time,$loop,$loop_every,$user);
			if($isSaved) {
	...
	<table class="table">
		...
		<tbody>
			<?php $a =1; 
			$notifList = $Sys->listNotification(); 
