...
*/
        	...
        	$Feb++;
    	        ...
    	        $sqlSum  = mysqli_query($conn," SELECT SUM(tk.no4) as total FROM tk_sales_sub as tk WHERE no2 IN (SELECT no2 FROM tk_sales_sub as t WHERE t.no2 = tk.no2 AND no7 != '' AND no5 NOT LIKE '%trial%' ) AND main_id = '".$rowFeb['main_id']."' AND tab_name = '".$rowFeb['tab_name']."' AND month = '".$rowFeb['month']."' AND no3 = 'R.F' AND row_no != '0'  ");    	        while($rowSum = mysqli_fetch_array($sqlSum)) {					$rf = number_format((float)$rowSum['total'], 2, '.', '');      	        }
