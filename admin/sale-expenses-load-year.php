            ...
            <table class="tbl-qa" style="width: 200px;float: left;">
              <thead>
            	<tr>
            	  ...
            	  <th class="table-header" style="font-size:14px;width:100px;border-right: 1px solid black;" ><center> Expenses </center></th>
            	</tr>
              </thead>
              <tbody id="bodyYear" >
              <?PHP
                $sql = " SELECT * FROM tk_sales_expenses WHERE main_id = '".$dataGrid['mainID']."' AND month !='' GROUP BY month ORDER BY FIELD(month,'Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec') ";
                    ...
                    ?>
                            <tr class="table-row" id="">
                                ...
                                <td onClick="editRow(this,'remove','bodyYear');" class="row-data" style="font-size:14px;width:100px;text-align: right;" contenteditable="true">
                                  <?PHP  
                                    $sqlSumRev = " SELECT SUM(amount) as total FROM tk_sales_expenses WHERE main_id = '".$dataGrid['mainID']."' AND month = '".$row['month']."' AND row_no != '0' ";
