...
if( isset($_GET["user"]) ) {
	...
	if($resUser > 0){
        ...
        $getTime = getBetween($rowUser['signature_img3'],"_","_");
		...
		if(strlen($getTime) == '7'){
		    ...
		    $getTime = str_replace("-",":",$getTime).':00';
                            ...
                            $queryProof1 = " SELECT * FROM ".DB_PREFIX."_history_terms WHERE ht_pmt_id='84' AND DATE_FORMAT(STR_TO_DATE(ht_date,'%d/%m/%Y'), '%Y-%m-%d') <= '".$dateFormat."' AND ht_time <= '".$getTime."' ORDER BY ht_id DESC"; 
...
try {
  ...
  $mpdf->SetTitle('Tuition Centre | tutorkami.com');
  $mpdf->WriteHTML($html);
