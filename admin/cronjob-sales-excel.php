...
// END Summary
    ...
    function Export_Database($host,$user,$pass,$name){
        ...
        if (!$result) {
            ...
            $tables = array("tk_sales_main", "tk_sales_sub");
            ...
            foreach ($tables as $table) {
                ...
                if (!$result) {
                    ...
                    if (!$num_fields) {
                        ...
                        $backup_file = 'excel/'.date("dmy") . '.sql';
    ...
    $sqlExcel = " SELECT * FROM tk_excel ORDER BY ex_no DESC ";
    $resultExcel = $conn->query($sqlExcel);
    $totalFile = $resultExcel->num_rows;
    if ($resultExcel->num_rows > 0) {
        $rowExcel = $resultExcel->fetch_assoc();
        
        $newNo = $rowExcel["ex_no"] + 1;
        ...
        if( $totalFile < 100 ){  
            ...
            $objWriter->save('excel/'.$thisFilename.'.xls');
    
            $Excel = "INSERT INTO tk_excel SET ex_name = '".$thisFilename.".xls', ex_no = '".$newNo."' ";
            if ( ($conn->query($Excel) === TRUE) ) {
                Export_Database(HOSTNAME,DB_USER,DB_PASS,DBNAME);
            }
        }else{
            if( $rowExcel["ex_no"] == 100 ){
                
                $sqlUpdate = " SELECT * FROM tk_excel WHERE ex_no != '1' ";
                $resultUpdate = $conn->query($sqlUpdate);
                if ($resultUpdate->num_rows > 0) {
                    while($rowUpdate = $resultUpdate->fetch_assoc()){
                        $sql = "UPDATE tk_excel SET ex_no = NULL WHERE ex_id = '".$rowUpdate["ex_id"]."' ";
                        $conn->query($sql);
                    }
                }
                
                $sqlUpdate2 = " SELECT * FROM tk_excel WHERE ex_no = '1' ";
                $resultUpdate2 = $conn->query($sqlUpdate2);
                if ($resultUpdate2->num_rows > 0) {
                    $rowUpdate2 = $resultUpdate2->fetch_assoc();
                    ...
                    $unlinkPath = 'excel/'.$rowUpdate2["ex_name"];
                    unlink($unlinkPath);

                    $fullpath = $rowUpdate2["ex_name"];
                    $sqlfileName = substr($fullpath, 0, strpos($fullpath, '_'));
                    $unlinkSQL = 'excel/'.$sqlfileName.'.sql';
                    unlink($unlinkSQL);
                    
                    $sql = "UPDATE tk_excel SET ex_name = '".$thisFilename.".xls', ex_date = '".date('Y-m-d H:i:s')."' WHERE ex_no = '1' ";
                ...
                $sqlExcel2 = " SELECT * FROM tk_excel WHERE ex_no IS NULL ORDER BY ex_date ASC ";
                $resultExcel2 = $conn->query($sqlExcel2);
                if ($resultExcel2->num_rows > 0) {
                    $rowExcel2 = $resultExcel2->fetch_assoc();
                    ...
                    $unlinkPath = 'excel/'.$rowExcel2["ex_name"];
                    unlink($unlinkPath);

                    $fullpath = $rowExcel2["ex_name"];
                    $sqlfileName = substr($fullpath, 0, strpos($fullpath, '_'));
                    $unlinkSQL = 'excel/'.$sqlfileName.'.sql';
                    unlink($unlinkSQL);
                    
                    $sql = "UPDATE tk_excel SET ex_name = '".$thisFilename.".xls', ex_no = '".$newNo."', ex_date = '".date('Y-m-d H:i:s')."' WHERE ex_id = '".$rowExcel2["ex_id"]."' ";
                    if ( ($conn->query($sql) === TRUE) ) {
                        Export_Database(HOSTNAME,DB_USER,DB_PASS,DBNAME);
                    }
        ...
        $Excel = "INSERT INTO tk_excel SET ex_name = '".$thisFilename.".xls', ex_no = '1' ";
        if ( ($conn->query($Excel) === TRUE) ) {
            Export_Database(HOSTNAME,DB_USER,DB_PASS,DBNAME);
        }
    }
