...
<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#tempModal">demo</button>-->
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer hidden" id="tempModalFooter">
        <!--<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button><button type="button" class="btn btn-primary">Save changes</button>-->
            <div class="spinner"><font style="font-size:14px;color:#900C3F"><b>Loading...</b></font><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>
      </div>
...
<div class="btn-group pull-right">
    <button type="button" class="btn btn-info-ori dropdown-toggle" data-toggle="dropdown">Tabs <span class="caret"></span></button>
    <ul class="dropdown-menu scrollable-menu" role="menu" style="width:150px;">
		<li><a class="btnSummary">Summary </a></li>
		<li><a class="btnExpenses">Expenses</a></li>
		<li><a class="btnBank">Bank</a></li>
		<li><a class="btnDeposit">Deposits</a></li>
	</ul>
</div>

<p class="hidden" id="demo123"></p>
    <div class="panel panel-default tab-pane" style="width:1920px;">
...
/*
echo '<br/>';
    $sqlTest = " SELECT * FROM tk_sales_sub WHERE main_id = '1' AND tab_name = 'Zahrah' AND month = 'Jun' AND row_no != '0' AND row_no = '999999' ORDER BY str_to_date(no1,'%d/%m/%Y') ASC, no2 ASC, case when cf is null then 1 else 0 end, cf ASC   ";
    $resultTest = $conn->query($sqlTest);
    if ($resultTest->num_rows > 0) {
        while($rowTest = $resultTest->fetch_assoc()){
            echo $rowTest['no1'].' | '.$rowTest['no2'].' | '.$rowTest['no3'].' | '.$rowTest['no4'].'<br/>';
        }
    }
 */       
?>
                ...
                if( receipt == 'Error!' ){
                    ...
                    if( type == 'Yes' || type == 'PV' ){
                        ...
                        newDate = dd + '/' + mm + '/' + yyyy;
                            ...
                            setTimeout(function() { $('#tempModal').modal({backdrop: 'static', keyboard: false}); }, 1500);        
                    }
    ...
    if( pJobID == '' ){
        ...
        if(/^\d+$/.test(pJobID)) {
            $.ajax({
                ...
                method: "POST",
                data: {action: 'proceedDeadline', pJobID: pJobID,pDeadline: pDeadline,pCheck: pCheck,pRemark: pRemark}, 
                        ...
                        if( pCheck !== true ){
                        	if(Lang2.trim() == 'BM'){
                                ...
                                document.body.removeChild(tempInput);
                              	...
                              	setTimeout(function() { $("#tempModal").modal(); }, 1000);
                        	}else{
                                var tempInput = document.createElement("input");
                                tempInput.style = "position: absolute; left: -1000px; top: -1000px";
...
/*
    $(window).scroll(function(){
        if($(document).scrollTop() > 200) {
            ...
            $('.scrollTop').css( {top:newPos});
        }
        else {
            $('.scrollTop').css( {top:200});
        }
    })
*/
    ...
    $(".btnTab").click(function () {
        ...
        $(this).addClass("active");   
        /*$(".btnTabMonth").removeClass("active");
        $(".btnTabMonth:first-child").addClass("active"); */
...
function undoCF(editableObj,column,id,numTable) {
    //alert( editableObj + ' ' + column + ' ' + id + ' ' + numTable );
    $("#confirmBox2").hide();
    setTimeout(function(){
...
function carryForwardConfirm2(editableObj,column,id,numTable) {
    //alert(id);
    /* note: CF 2 = Table ada CF/Takda CF (tak termasuk last row)    */
    var mainID  = document.getElementById('mainID').value;
    var btnTab      = $(".btnTab.active").text();
    var btnTabMonth = $(".btnTabMonth.active").text();
...
function carryForwardLastRowConfirm(editableObj,column,id,numTable) {
    /*alert('aaaaa');*/
    /* note: CF 3 = Table ada CF/Takda CF (last row punya - tak termasuk CF atas)    */
...
function carryForwardConfirm(editableObj,column,id,numTable) {
    /*alert('3');*/
    /* note: CF 1 = Table ada CF/Takda CF (tak termasuk last row)    */
    var mainID  = document.getElementById('mainID').value;
    var btnTab      = $(".btnTab.active").text();
    var btnTabMonth = $(".btnTabMonth.active").text();
    var trythis = (Number(numTable) +1);
		        ...
		        if( trimData.includes("R.F") ){
                    ...
                    $.ajax({
                        ...
                        success:function(result){
                            ...
                            /*var x = confirm("Are you sure you want to issue receipt?");
                            if (x == true){
                                $.ajax({
                                    type:'POST',
                                    url:'sale-issue-receipt.php',
                                    data: {
                                        dataReceipt: {id: numberOnly},
                                    },
                                    success:function(receipt){
                                        alert(receipt);
                                    }
...
/*
		    var stringOnly = data.trim();
		    stringOnly = stringOnly.replace(/[0-9]/g, '');
		    
		    var numberOnly = data.trim();
		    numberOnly = numberOnly.replace( /^\D+/g, '');
		    
		    $(".overlayBG").hide();
		    if( stringOnly.trim() == 'Success' ){
		        $("#add-more").show();
		        document.getElementById('duplicateBtn').innerHTML = '';
                $.ajax({
                    type:'POST',
...
/**/                 		    
                		    if( RF > 0 ){
                            	...
                            	'<td style="font-size:14px;" >  <input type="hidden" id="app_RFrow_no" value="'+newNo+'" /> <input type="hidden" id="app_RFno1" value="'+today+'"/> <input type="hidden" id="app_RFno2" value="'+JobID+'"/> <input type="hidden" id="app_RFno3" value="R.F"/> <input type="hidden" id="app_RFno4" value="'+RF+'" /> <input type="hidden" id="app_RFno5" /> <input type="hidden" id="app_RFno6" /> <input type="hidden" id="app_RFno7" /> <input type="hidden" id="app_RFno8" /> <input type="hidden" id="app_RFno9" /> <input type="hidden" id="app_RFno10" /> <input type="hidden" id="app_RFno11" value="NJ"/>  <span id="confirmAdd"><a onclick="app_RemoveAdd();" class="ajax-action-links" style="color:red"><b>Remove</b></a></span>  </td>' +	
                            	'</tr>';
                            	//$('table > tbody > tr').eq(newNo - 2).after(data); 
                            	$('table > #table-body2 > tr').eq(newNo - 2).after(data); 
...
function addRowConfirm(editableObj,column,id,row_no) {
    ...
    setTimeout(function(){
        ...
        //alert(trythis + ' ' + btmCells[3].innerText);
        ...
        document.getElementById('duplicateBtn').innerHTML = '<span id="confirmAdd"><a onClick="app_addToDatabase()" class="ajax-action-links" style="color:#28A745"><b>Save</b></a> / <a onclick="app_cancelAdd();" class="ajax-action-links" style="color:#007BFF"><b>Cancel</b></a></span>';
        	...
        	'</tr>';
        	//$('table > tbody > tr').eq(1).after(data); 
		        ...
		        if( trimData.includes("R.F") ){
                    $.ajax({
                        ...
                        success:function(result){
                            ...
                            var elmnt = document.getElementById("loadFooterSale"); elmnt.scrollIntoView();
                        }
                    });
		        }else{
                    $.ajax({
...
function dateTutorPaid(editableObj,column,id,tableID) {
    ...
    //alert( no1 + ' ' + no2 + ' ' + no3 + ' ' + no4 + ' ' + no5   + ' ' + no6 + ' ' + no7 + ' ' + no8   + ' ' + no9   + ' ' + no10   + ' ' + no11  );
...
// KOMEN : Guna bila masuk data di right side and data belum ada. Untuk cf/normal table. Untuk isu PV.
function show(editableObj,column,id,tableID) {
    ...
    var no10 = x[parseInt(10,10)].innerHTML.trim();
    var no11 = x[parseInt(11,10)].innerHTML.trim();
    //alert(id + ' ' +btnTabMonth + ' ' +no6 + ' ' +no7 + ' ' +no9 + ' ' +no11);
    ...
    $.ajax({
        ...
        success: function(data){
		    ...
		    if( numberOnly != '' ){
		        ...
		        /*
		        var x = confirm("Do you want to issue PV & send WA to tutor?");
		        if (x == true){
		            $(".overlayBG").show();
		            $.ajax({
		                type:'POST',
...
// KOMEN : function ni guna, bila data dah ada di DB dan untuk edit rekod tu. Dari saveToDatabase. Untuk cf/mormal table
function saveThis(id,column,btnTabMonth,currVal,no9) {
    //alert( id + ' ' + column + ' ' + btnTabMonth + ' ' + currVal + ' ' + no9 );
    var mainID  = document.getElementById('mainID').value;
    var btnTab      = $(".btnTab.active").text();
    var btnTabMonth = $(".btnTabMonth.active").text();
                ...
                $.ajax({
                    ...
                    success:function(result){
                        ...
                        $.ajax({
                            ...
                            success:function(resultFooter){
                                document.getElementById("loadFooterSale").innerHTML = resultFooter;
                                document.getElementById('myDIV123').scrollTop = document.getElementById ("demo123").innerHTML;
...
// KOMEN : function ni guna, bila data dah ada di DB dan untuk edit rekod tu. Call function saveThis (atas ni). Untuk cf/mormal table
...
function myFunction123() {
  ...
  //document.getElementById ("demo123").innerHTML = "Horizontally: " + x + "px<br>Vertically: " + y + "px";
...
function saveExpenses(editableObj,column,id,tableID) {
    ...
    timer = setTimeout(function(){
        ...
        //alert( id + ' ' + column + ' ' + btnTabMonth + ' ' + currVal );
    }, 1000);
}
...
/*
        	...
        	'<td style="font-size:14px;background-color: #F3EBCD;" contenteditable="true" id="txt_no1"     onBlur="addToHiddenField(this,\'no1\')"    onClick="editRow(this);" >'+today+'</td>' +
        	'<td style="font-size:14px;background-color: #F3EBCD;" contenteditable="true" id="txt_no2"     onBlur="addToHiddenField(this,\'no2\')"    onClick="editRow(this);" onkeyup="keyupFunction(this,this.id)"></td>' +
        
        	'<td style="font-size:14px;background-color: #F3EBCD;" contenteditable="true" id="txt_no3"     onBlur="addToHiddenField(this,\'no3\')"    onClick="editRow(this);" onkeyup="keyupFunction(this,this.id)"></td>' +
        	'<td style="font-size:14px;background-color: #F3EBCD;" contenteditable="true" id="txt_no4"     onBlur="addToHiddenField(this,\'no4\')"    onClick="editRow(this);" onkeyup="keyupFunction(this,this.id)"></td>' +
        	

        
        	'<td style="font-size:14px;" >  <input type="hidden" id="row_no" value=""/> <input type="hidden" id="no1" value="'+today+'"/> <input type="hidden" id="no2" /> <input type="hidden" id="no3" /> <input type="hidden" id="no4" /> <span id="confirmAdd">   </span>  </td>' +	
        	'</tr>';
          $("#table-bodyExpenses").append(data);
*/
        ...
        success: function(data){
		    ...
		    if( numberOnly != '' ){
		        $.ajax({
		            ...
		            success:function(result){
		                document.getElementById("loadSaleData").innerHTML = result;
		                document.getElementById('bodyDeposit'+numberOnly).click();
		            }
		        });
		    }else{
		        alert(trimData);
