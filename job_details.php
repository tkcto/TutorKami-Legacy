...
if( (isset($_GET['jid']) && $_GET['jid'] > 0) && (isset($_GET['status']) && $_GET['status'] != '')  ){
	$qJob = " SELECT * FROM tk_job WHERE j_id='".$_GET['jid']."' "; 
    $resultqJob = $conn->query($qJob);
    if ($resultqJob->num_rows > 0) {
        $rowqJob = $resultqJob->fetch_assoc();
		if( $rowqJob['j_status'] == $_GET['status']){
			?>
...
<?PHP
if( hasWord('ONLINE', $row->jt_subject) ){
?>
							 <tr>
								<td class="org-txt "><strong><?php echo 'Type'; ?> :</strong></td>
...
?>
					   ...
					   <hr>
								...
								if ($tablet_browser > 0) {
									//print 'is tablet';
									include('job_details-desktop.php');
								}else if ($mobile_browser > 0) {
									//print 'is mobile';
									include('job_details-mobile.php');
								}else {
									include('job_details-desktop.php');
								}   
								?>
			...
			<?PHP
			  ...
			  ?>
			  <div class="container">
				 <h1 class="text-center text-uppercase blue-txt"><?php echo JOB_DETAILS; ?></h1><hr><br/><br/>
				 <div class="col-md-10 col-md-offset-1 ">
					<div class="alert alert-danger" role="alert"><center>We're sorry, the job has <b>closed</b>. You can no longer apply for this job</center></div>
					<p><b><center>If you want to see the details of this job, please click <a style="cursor: pointer;" href="https://www.tutorkami.com/job_details?jid=<?PHP if( (isset($_GET['jid']) && $_GET['jid'] > 0) ){ echo $_GET['jid'];} ?>&status=closed" target="_blank" >here</a></center></b></p>
				 </div>
			  </div><br/><br/><br/><br/>
			  <?PHP
		}
...
<button type="button" class="hidden btn btn-primary" data-toggle="modal" data-target="#myModalShowWa">
  ..
</button>
  <div id="myModalShowWa" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
    
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-body">
          <p><center>Your profile will be sent to the client now. Please subscribe to our What’s App auto message, so we can provide you the Client’s name.
          <br><br>
          <button type="button" class="btn btn-oren" data-dismiss="modal" onClick="openLink()">To Subscribe, click this button & click send at What'sApp</button><br><br>
          <button type="button" class="btn btn-secondary" data-dismiss="modal" onClick="shoSuccess()">Not right now</button>
          </center></p>
        </div>
...
?>
			<div class="cd-signin-modal__block js-signin-modal-block" data-type="login" > <!-- log in form -->
				<form class="cd-signin-modal__form">
					...
					?>
						<p class="cd-signin-modal__fieldset">
							<button type="button" class="btn btn-oren pull-right oriButton" style="margin-top:-20px;" onclick="submitRate()">Submit</button>
							<button type="button" disabled class="hidden btn btn-oren pull-right fakeButton " style="margin-top:-20px;"><i class="fa fa-refresh fa-spin"></i> Submit</button>
						</p>
					<?PHP
					}else{
					?>
			...
			<div class="cd-signin-modal__block js-signin-modal-block" data-type="signup"> 
				<form id="addClass" class="cd-signin-modal__form">
					<center>
						...
						<label id="btmProceedApply" type="button" class="btn btn-oren btn-xs oriButton" onClick="proceedApply()">Yes, proceed to apply this job</label>
						<label type="button" disabled class="hidden btn btn-oren btn-xs fakeButton" onClick="proceedApply()"><i class="fa fa-refresh fa-spin"></i> Yes, proceed to apply this job</label>
					</p></center>
...
function applyJob() {
    ...
    $(".fakeButton").removeClass("hidden");
		...
		$.ajax({
			...
			success:function(result){
				if(result == 'done'){
				    $(".oriButton").removeClass("hidden");
				    $(".fakeButton").addClass("hidden");

                		$.ajax({
                			type:'POST',
                			url:'notsubscribe.php',
                			data:{thisuser: thisuser},
                			success:function(result){
                			    if( result == 'yes' ){
                			        $('#myModalShowWa').modal('show');
                			    }else if( result == 'no' ){
                			        $('#myModalSuccess').modal('show');
                			    }else{
                			        alert(result);
