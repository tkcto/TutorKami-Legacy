...
if(!empty($_POST["mainID"])) {
    ...
    if( (isset($_SESSION['tk']['u_id'])) && ($_SESSION['tk']['u_id'] == '1' || $_SESSION['tk']['u_id'] == '8' || $_SESSION['tk']['u_id'] == '1579926' || $_SESSION['tk']['u_id'] == '1581081' || $_SESSION['tk']['u_id'] == '3') ){
        ...
        if( $_POST["addRowBetween"] == 'Yes' ){
            ...
            if( $_POST["RFno2"] != '' ){
                $OldNo = " SELECT id, main_id, tab_name, month, row_no FROM tk_sales_sub WHERE main_id = '".$_POST["mainID"]."' AND tab_name = '".$_POST["btnTab"]."' AND month = '".$_POST["btnTabMonth"]."' AND row_no >= '".$_POST["row_no"]."' AND row_no != '0' AND row_no != '999999'    ";
                $resultOldNo = $conn->query($OldNo);
                if ($resultOldNo->num_rows > 0) {
                    while($rowOldNo = $resultOldNo->fetch_assoc()){
                        $updateNo = "UPDATE tk_sales_sub SET row_no = '".($rowOldNo["row_no"]+2)."' WHERE id = '".$rowOldNo["id"]."' ";
                        $conn->query($updateNo);                    
                    }
                }
                $sqlInsert = " INSERT INTO tk_sales_sub (main_id, tab_name, month, row_no, no1, no2, no3, no4, no5, no6, no7, no8, no9, no10, no11) VALUES 
                ('".trim($_POST["mainID"])."', '".trim($_POST["btnTab"])."', '".trim($_POST["btnTabMonth"])."', '".trim($_POST["row_no"])."', '".trim($_POST["no1"])."', '".trim($_POST["no2"])."', '".htmlspecialchars(trim($_POST["no3"]), ENT_QUOTES)."', '".trim($_POST["no4"])."', '".$conn -> real_escape_string(trim($_POST["no5"]))."', '".trim($_POST["no6"])."', '".trim($_POST["no7"])."', '".trim($_POST["no8"])."', '".trim($_POST["no9"])."', '".trim($_POST["no10"])."', '".$conn -> real_escape_string(trim($_POST["no11"]))."') ";
                if ( ($conn->query($sqlInsert) === TRUE) ) {
                    ...
                    ('".trim($_POST["mainID"])."', '".trim($_POST["btnTab"])."', '".trim($_POST["btnTabMonth"])."', '".trim($_POST["row_no"]+1)."', '".trim($_POST["RFno1"])."', '".trim($_POST["RFno2"])."', '".trim($_POST["RFno3"])."', '".trim($_POST["RFno4"])."', '".$_POST["RFno5"]."', '".trim($_POST["RFno6"])."', '".trim($_POST["RFno7"])."', '".trim($_POST["RFno8"])."', '".trim($_POST["RFno9"])."', '".trim($_POST["RFno10"])."', '".trim($_POST["RFno11"])."') ";
                ...
                if ($resultOldNo->num_rows > 0) {
                    while($rowOldNo = $resultOldNo->fetch_assoc()){
                        $updateNo = "UPDATE tk_sales_sub SET row_no = '".($rowOldNo["row_no"]+1)."' WHERE id = '".$rowOldNo["id"]."' ";
                        $conn->query($updateNo);                    
                    }
                }
                $sqlInsert = " INSERT INTO tk_sales_sub (main_id, tab_name, month, row_no, no1, no2, no3, no4, no5, no6, no7, no8, no9, no10, no11) VALUES 
                        ...
                        if( trim($_POST["no3"]) != 'R.F' ){
                                ...
                                if ($resultGetJob->num_rows > 0) {
                                    ...
                                    $my_new_date = '20'.$aDate[2].'-'.$aDate[1].'-'.$aDate[0];
                                        ...
                                        /* insert payment */
                                        $insertPayment = " INSERT INTO tk_payment_history (ph_user_type, ph_user_id, ph_date, ph_job_id, ph_receipt, ph_amount, ph_rf, hour, tutor) VALUES  
                                        ('4', '".$rowGetJob['u_id']."', '".$my_new_date."', '".trim($_POST["no2"])."', '".$NoCycle."', '".trim($_POST["no4"])."', '0.00', '".trim($_POST["no10"])."', '".$displayname."') ";
                                        $conn->query($insertPayment);
                                        $last_id = $conn->insert_id;
                                        /* insert payment */    
