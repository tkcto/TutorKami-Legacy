			...
			if($resultGeJob->num_rows > 0){
				...
				if( $rowGeJob['j_status'] == 'open' ){
    				if( $rowGeJob['j_deadline'] != '' && $rowGeJob['j_deadline'] != '0000-00-00' ){
    					...
    					$sqlGetClient = "SELECT u_status, u_id, signature_img, signature_img2, ud_first_name FROM tk_user INNER JOIN tk_user_details ON ud_u_id = u_id WHERE u_status = 'A' AND u_id = '".$rowGeJob['u_id']."' ";
                                                ...
                                                if( !activeAPI( $website ) ) {
                        					        ...
                        					        if ($resultLogWa->num_rows > 0) {
                            					        ...
                            					        if($make_call){
                            					            ...
                            					            $sqlWaNoti2 = "INSERT INTO tk_whatsapp_noti SET wa_job_id = '".$ajobid."', wa_user = '".$rowGeTutor['u_id']." - ".$rowGeTutor['u_email']."', wa_remark = 'Apply Job', wa_status = 'POST', wa_note = '".$make_call."', wa_date = '".date('Y-m-d H:i:s')."' ";
            					    ...
            					    if( !activeAPI( $website ) ) {
            					        ...
            					        if ($resultLogWa->num_rows > 0) {
            					            ...
            					            if( $rowLogWa['wa_note'] == 'Yes' ){
                    					        ...
                    					        $args->content = "Hi ".ucwords($rowGetClient['ud_first_name'])." we have a tutor who is interested to teach the student. This is the link to profile ".$TutorDisName." https://www.tutorkami.com/tutor_profile?did=".$TutorDisID."\r\nYou can message ".$TutorDisName." by clicking this link https://wa.me/".$TutorPhoneNo." ";
                    					        $xdata->args = $args;
                    					        $make_call = callAPI('POST', 'https://wa.tutorkami.my/sendText', $xdata );
                    					        if($make_call){
                    					            $sqlWaNoti = "INSERT INTO tk_whatsapp_noti SET wa_job_id = '".$ajobid."', wa_user = '".$rowGeJob['u_id']." - ".$rowGeJob['j_email']."', wa_remark = 'Apply Job', wa_status = 'POST', wa_note = '".$make_call."', wa_date = '".date('Y-m-d H:i:s')."' ";
                    					            $exeWaNoti = $conn->query($sqlWaNoti);
                    					        }else{
                    					        ...
                    					        if($make_call){
                    					            ...
                    					            $sqlWaNoti2 = "INSERT INTO tk_whatsapp_noti SET wa_job_id = '".$ajobid."', wa_user = '".$rowGeJob['u_id']." - ".$rowGeJob['j_email']."', wa_remark = 'Apply Job', wa_status = 'POST', wa_note = '".$make_call."', wa_date = '".date('Y-m-d H:i:s')."' ";
                    					            $exeWaNoti2 = $conn->query($sqlWaNoti2);
                    					        }else{
                    					            $sqlWaNoti = "INSERT INTO tk_whatsapp_noti SET wa_job_id = '".$ajobid."', wa_user = '".$rowGeJob['u_id']." - ".$rowGeJob['j_email']."', wa_remark = 'Apply Job', wa_status = 'POST', wa_note = 'Not Sent', wa_date = '".date('Y-m-d H:i:s')."' ";
                    					            $exeWaNoti = $conn->query($sqlWaNoti);
