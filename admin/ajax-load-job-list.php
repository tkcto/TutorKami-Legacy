...
$host = sprintf("mysql:host=%s;dbname=%s", HOSTNAME, DBNAME);
$connect = new PDO($host, DB_USER, DB_PASS);
...
$column = array(
    'j_id', 'j_create_date', 'j_start_date', 'jlt_title', 'jt_subject', 'j_area', 'st_name',
    'j_status', 'j_payment_status', 'aj_j_id', 'j_deadline'
);
...
if (isset($_POST['j_email']) && $_POST['j_email'] != '') {
    $query .= ' AND j_email = "' . $_POST['j_email'] . '"';
}

if (isset($_POST['j_rate']) && $_POST['j_rate'] != '') {
    $query .= ' AND j_rate LIKE "' . $_POST['j_rate'] . '%"';
}

if (isset($_POST['j_hired_tutor_email']) && $_POST['j_hired_tutor_email'] != '') {
    $query .= ' AND j_hired_tutor_email = "' . $_POST['j_hired_tutor_email'] . '"';
}

if (isset($_POST['j_telephone']) && $_POST['j_telephone'] != '') {
    $query .= ' AND (j_telephone = "' . $_POST['j_telephone'] . '" OR j_telephone_alt = "' . $_POST['j_telephone'] . '" )';
}

if (isset($_POST['j_date']) && $_POST['j_date'] != '') {
    $query .= ' AND j_create_date = "' . $_POST['j_date'] . '"';
}

if (isset($_POST['deadlineYear']) && $_POST['deadlineYear'] != '') {
    $query .= ' AND j_create_date LIKE "' . $_POST['deadlineYear'] . '%"';
}

if (isset($_POST['j_jl_id']) && $_POST['j_jl_id'] != '') {
    $query .= ' AND j_jl_id = "' . $_POST['j_jl_id'] . '"';
}
...
if (isset($_POST['j_deadline']) && $_POST['j_deadline'] != '') {
    $query .= ' AND j_deadline = "' . $_POST['j_deadline'] . '"';
}

if (isset($_POST['j_start_date']) && $_POST['j_start_date'] != '') {
    $query .= ' AND j_start_date = "' . $_POST['j_start_date'] . '"';
}

if (isset($_POST['j_end_date']) && $_POST['j_end_date'] != '') {
    $query .= ' AND j_end_date = "' . $_POST['j_end_date'] . '"';
}

if (isset($_POST['j_creator_email']) && $_POST['j_creator_email'] != '') {
    $query .= ' AND j_creator_email = "' . $_POST['j_creator_email'] . '"';
}

if (isset($_POST['sort_by']) && $_POST['sort_by'] == "12") {
    $query .= ' AND (j_deadline IS NULL OR j_deadline = "0000-00-00")';
}
...
foreach ($result as $row) {
    ...
    //$sub_array[] = $row['j_id'];
    $sub_array[] = '<a href="job-edit.php?j=' . $row['j_id'] . '">' . $row['j_id'] . '</a>';
...
function count_all_data($connect) {
    ...
    if (isset($_POST['j_telephone']) && $_POST['j_telephone'] != '') {
        $query .= ' AND j_telephone = "' . $_POST['j_telephone'] . '"';
    }
...
$output = array(
    "draw"            => intval($_POST["draw"]), "recordsTotal" => count_all_data($connect),
    "recordsFiltered" => $number_filter_row, "data" => $data
);
