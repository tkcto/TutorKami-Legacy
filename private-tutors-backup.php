...
if( strpos($actual_link, '?/') !== false ){
	...
	$seoTitle = $seoSub.' '.$seoLevel.' Tutors for Home Tuition in '.$seoArea.' & Nearby Areas';
	$seoDescription = 'Get '.$seoSub.' '.$seoLevel.' private tutors for home tuition in '.$seoArea.' & nearby areas easier, safer & with a guarantee';

	$seoLevel2 = "Pre School, Tahun 1-3, UPSR, PT3, SPM ";
	$seoSubject2 = $seoSub.' ';
	$contentTitle = "tutors and tuition teachers in ";
	$seoArea2 = "Private Tutor for Home Tuition or Home School in ".$seoArea.' ';
	
}else{
	$seoTitle = 'English Mathematics Level SPM UPSR Tutors for Home Tuition in Malaysia & Nearby Areas';
	$seoDescription = 'Get English Mathematics Level Pre School, Tahun 1-3, UPSR, PT3, SPM private tutors for home tuition in Malaysia & nearby areas easier, safer & with a guarantee';
	
	$seoLevel2 = "Pre School, Tahun 1-3, UPSR, PT3, SPM ";
	$seoSubject2 = "Bahasa melayu, bahasa inggeris ";
	$contentTitle = "tutors and tuition teachers in ";
	$seoArea2 = "Private Tutor for Home Tuition or Home School in Malaysia ";
...
if($resultState->num_rows > 0){ 
	while($rowState = $resultState->fetch_assoc()){  
		?><option value="<?php echo $rowState['st_id']; ?>" <?php if ( strcasecmp($stateSelected == $rowState['st_id']) || strcasecmp($stateSelected == $rowState['st_name']) ) echo 'selected' ; ?> ><?php echo $rowState['st_name']; ?></option><?php
	} 
}
/*
...
/*
                        ...
                        <select class="form-control" name="tutor_course[]" id="level_drop">
                           ...
                           <option value="All"><?php echo "All"; ?></option>
                           <?php 
$querySubject = "SELECT * FROM tk_tution_course ORDER BY tc_id ASC"; 
$resultSubject = $dbCon->query($querySubject); 
if($resultSubject->num_rows > 0){ 
	while($rowSubject = $resultSubject->fetch_assoc()){  
		?><option value="<?php echo $rowSubject['tc_id']; ?>" <?php if ( strcasecmp($levelSelected == $rowSubject['tc_id']) || strcasecmp($levelSelected == $rowSubject['tc_title']) ) echo 'selected' ; ?> ><?php echo $rowSubject['tc_title']; ?></option><?php
	} 
}
         ...
         <div class="col-md-12">
			<div class="job-table" style="margin-top:30px;">
				<div class="panel panel-default">
					...
					<!--<h3 class="text-primary text-left"><?PHP //echo ucfirst($seoLevel2); ?> <?PHP //echo strtolower($seoSubject2); ?> <?PHP //echo $contentTitle; ?> <?PHP //echo str_replace("Private Tutor for Home Tuition or Home School in"," ",$seoArea2); ?>
					</h3>
					<h4 class="text-left"><?PHP //echo "We help you get the best ".strtolower($seoSubject2)."".strtolower($seoLevel2)."".strtolower($seoArea2)." area."; ?></h4>-->
...
*/
			/*currSub = strSub.replace(/%20/g, '_');
			currLevel = strLevel.replace(/%20/g, '_');
			currArea = strArea.replace(/%20/g, '_');*/
