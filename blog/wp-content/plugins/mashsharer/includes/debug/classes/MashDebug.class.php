...
if ( class_exists( "MashDebug" ) )
	return;

class MashDebug implements iMashDebug {
	...
	private function _get_interface( $interface_name ) {
		...
		switch ( $interface_name ) {
			...
			default:
				...
				if ( $interface && !in_array( 'iMashDebug', class_implements( $interface ) ) )
					$interface = null;
...
if ( !function_exists( 'mashdebug' ) ) {
	/**
	 * Returns a MashDebug instance.
	 *
	 * @return MashDebug
	 */
	function mashdebug() {
		return MashDebug::instance();
	}
}
