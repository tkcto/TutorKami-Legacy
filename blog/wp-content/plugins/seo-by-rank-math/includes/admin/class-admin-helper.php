...
class Admin_Helper {
	...
	public static function register_product( $username, $password ) {
		...
		if ( false !== $body && isset( $body['login'] ) ) {
			...
			GlobalHelper::add_notification( esc_html__( 'Thank you for connecting Rank Math to your MyThemeShop account.', 'rank-math' ), 'success' );
	...
	private static function authenticate_user( $username, $password ) {
		$response = wp_remote_post( 'https://mythemeshop.com/mtsapi/v1/get_key/', [
			'timeout' => 10,
			'body'    => [
				'user' => $username,
				'pass' => $password,
	...
	public static function get_social_share() {
		...
		$tw_link = 'https://link.mythemeshop.com/sYmtFD3u';
		$fb_link = urlencode( 'https://link.mythemeshop.com/w74ifiZ1' );
