...
if( (isset($_SESSION['tk']['u_id'])) && ($_SESSION['tk']['u_id'] == '1' || $_SESSION['tk']['u_id'] == '8' || $_SESSION['tk']['u_id'] == '1579926' || $_SESSION['tk']['u_id'] == '1581081' || $_SESSION['tk']['u_id'] == '3') ){
    ...
    if($result->num_rows > 0){
        ...
        $header = date('d/m/Y H:i:s')." | DELETE ( Expenses - ".trim($_POST["btnTabMonth"])." ".$row['row_no'].") | ".$adminName;
        $header .= "\n ### Date   : ".trim($row['date']);
        $header .= "\n ### Item   : ".trim($row['item']);
        ...
        if( $header != '' ){
            ...
            $delete= " DELETE FROM tk_sales_expenses WHERE id = '".$row['id']."'  ";
            if ($conn->query($delete) === TRUE) {
                
                $ThisRow = 1;
                $GetRow = " SELECT id, main_id, month, row_no FROM tk_sales_expenses WHERE main_id = '".$row['main_id']."' AND month = '".$row['month']."' ORDER BY id ASC "; 
                $resultGetRow = $conn->query($GetRow); 
                if($resultGetRow->num_rows > 0){
                    while($rowGetRow = $resultGetRow->fetch_assoc()){
                        $Update= " UPDATE tk_sales_expenses set row_no = '".$ThisRow."' WHERE id = '".$rowGetRow['id']."'  ";
                        $conn->query($Update);
                        $ThisRow++;
                    }
                }
