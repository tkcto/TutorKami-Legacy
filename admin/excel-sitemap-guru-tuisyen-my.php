...
if ($resultState->num_rows > 0) {
    ...
    while($rowState = $resultState->fetch_assoc()){
                ...
                $resultIn = 'https://www.tutorkami.my/guru-tuisyen.php?/'.$stateName;
    ...
    while($rowState = $resultState->fetch_assoc()){
        ...
        if ($resultCity->num_rows > 0) {
            while($rowCity = $resultCity->fetch_assoc()){
                ...
                $resultIn = 'https://www.tutorkami.my/guru-tuisyen.php?/'.$cityName.'/'.$stateName;
...
if ($resultState->num_rows > 0) {
    ...
    while($rowState = $resultState->fetch_assoc()){
        ...
        $qCity = " SELECT * FROM tk_tution_subject WHERE ts_tc_id = '".$rowState['tc_id']."' ORDER BY ts_description ASC ";
        $resultCity = $conn->query($qCity);
        if ($resultCity->num_rows > 0) {
            while($rowCity = $resultCity->fetch_assoc()){
            ...
            while($rowCity = $resultCity->fetch_assoc()){
                ...
                if ($resultLevel->num_rows > 0) {
                    while($rowLevel = $resultLevel->fetch_assoc()){
                        ...
                        $resultIn = 'https://www.tutorkami.my/guru-tuisyen.php?/'.$LevelName.'/'.$cityName.'/'.$stateName;
            ...
            while($rowCity = $resultCity->fetch_assoc()){
                ...
                if ($resultLevel->num_rows > 0) {
                    while($rowLevel = $resultLevel->fetch_assoc()){
                        ...
                        $qCourse = " SELECT * FROM tk_tution_subject WHERE ts_tc_id ='".$rowLevel['tc_id']."' ORDER BY ts_description ASC ";
                        ...
                        if ($resultCourse->num_rows > 0) {
                            while($rowCourse = $resultCourse->fetch_assoc()){
                                ...
                                $resultIn = 'https://www.tutorkami.my/guru-tuisyen.php?/'.$CourseName.'/'.$LevelName.'/'.$cityName.'/'.$stateName;
...
// START STATE, CITY, LEVEL & COURSE ONLY
  ...
  header('Content-Disposition: attachment; filename=auto-sitemap-guru-tuisyen-my.xml');
  echo $output;
